...
#     routine.
from rithmic_api import base_pb2
from rithmic_api import request_account_list_pb2
from rithmic_api import response_account_list_pb2
from rithmic_api import request_heartbeat_pb2
from rithmic_api import request_rithmic_system_info_pb2
from rithmic_api import response_rithmic_system_info_pb2
from rithmic_api import request_login_pb2
from rithmic_api import response_login_pb2
from rithmic_api import request_login_info_pb2
from rithmic_api import response_login_info_pb2
from rithmic_api import request_logout_pb2
from rithmic_api import request_trade_routes_pb2
from rithmic_api import response_trade_routes_pb2
from rithmic_api import request_subscribe_for_order_updates_pb2
from rithmic_api import request_new_order_pb2
from rithmic_api import exchange_order_notification_pb2
from rithmic_api import rithmic_order_notification_pb2
...
async def rithmic_order_notification_cb(msg_buf):
    ...
    notify_type_to_string = {
        rithmic_order_notification_pb2.RithmicOrderNotification.ORDER_RCVD_FROM_CLNT: "ORDER_RCVD_FROM_CLNT",
    ...
    transaction_type_to_string = {
        rithmic_order_notification_pb2.RithmicOrderNotification.TransactionType.BUY: "BUY",
        rithmic_order_notification_pb2.RithmicOrderNotification.TransactionType.SELL: "SELL",
    }

    duration_to_string = {
        rithmic_order_notification_pb2.RithmicOrderNotification.Duration.DAY: "DAY",
    ...
    price_type_to_string = {
        rithmic_order_notification_pb2.RithmicOrderNotification.PriceType.LIMIT: "LIMIT",
    ...
    order_placement_to_string = {
        rithmic_order_notification_pb2.RithmicOrderNotification.OrderPlacement.MANUAL: "LIMIT",
        rithmic_order_notification_pb2.RithmicOrderNotification.OrderPlacement.AUTO: "AUTO",
    }

    response_data = {
        "template_id": msg.template_id,
        "notify_type": f"{notify_type_to_string[msg.notify_type]}({msg.notify_type})",
        "is_snapshot": msg.is_snapshot,
        "status": msg.status,
        "basket_id": msg.basket_id,
        "original_basket_id": msg.original_basket_id,
        "fcm_id": msg.fcm_id,
        "ib_id": msg.ib_id,
        ...
        "trade_exchange": msg.trade_exchange,
        "trade_route": msg.trade_route,
        "exchange_order_id": msg.exchange_order_id,
        "instrument_type": msg.instrument_type,
        "quantity": msg.quantity,
        "price": msg.price,
        "trigger_price": msg.trigger_price,
        "transaction_type": f"{transaction_type_to_string[msg.transaction_type]}({msg.transaction_type})",
        "duration": f"{duration_to_string[msg.duration]}({msg.duration})",
        "price_type": f"{price_type_to_string[msg.price_type]}({msg.price_type})",
        "orig_price_type": f"{price_type_to_string[msg.orig_price_type]}({msg.orig_price_type})",
        "manual_or_auto": f"{order_placement_to_string[msg.manual_or_auto]}({msg.manual_or_auto})",
        "sequence_number": msg.sequence_number,
        "orig_sequence_number": msg.orig_sequence_number,
        "cor_sequence_number": msg.cor_sequence_number,
        "currency": msg.currency,
        "country_code": msg.country_code,
        "text": msg.text,
        "report_text": msg.report_text,
        "remarks": msg.remarks,
        "ssboe": msg.ssboe,
        "usecs": msg.usecs,
    }
    ...
    if msg.status == "complete" or msg.notify_type == rithmic_order_notification_pb2.RithmicOrderNotification.COMPLETE:
...
async def exchange_order_notification_cb(msg_buf):
    ...
    notify_type_to_string = {
        exchange_order_notification_pb2.ExchangeOrderNotification.STATUS: "STATUS",
    ...
    transaction_type_to_string = {
        exchange_order_notification_pb2.ExchangeOrderNotification.TransactionType.BUY: "BUY",
        exchange_order_notification_pb2.ExchangeOrderNotification.TransactionType.SELL: "SELL",
    }

    duration_to_string = {
        exchange_order_notification_pb2.ExchangeOrderNotification.Duration.DAY: "DAY",
    ...
    price_type_to_string = {
        exchange_order_notification_pb2.ExchangeOrderNotification.PriceType.LIMIT: "LIMIT",
    ...
    order_placement_to_string = {
        exchange_order_notification_pb2.ExchangeOrderNotification.OrderPlacement.MANUAL: "LIMIT",
...
async def consume(ws):
    ...
    while num_msgs < max_num_msgs and g_order_is_complete == False:
        ...
        if base.template_id == 13:
            ...
            resp = await rithmic_order_notification_cb(msg_buf)
...
async def list_accounts(ws, fcm_id, ib_id, user_type):
    ...
    while rp_is_done == False:
        ...
        if (
            g_rcvd_account == False
            and len(rp.rq_handler_rp_code) > 0
            and rp.rq_handler_rp_code[0] == "0"
            and len(rp.fcm_id) > 0
            and len(rp.ib_id) > 0
            and len(rp.account_id) > 0
        ):
            g_fcm_id = rp.fcm_id
            g_ib_id = rp.ib_id
...
async def new_order(ws, fcm_id, ib_id, account_id, exchange, symbol, trade_route, side, quantity=1):
...
async def place_order(uri, system_name, user_id, password, g_exchange, g_symbol, g_side, quantity):
    resp = None
    ssl_context = None
    if "wss://" in uri:
    ...
    if g_rcvd_account and g_rcvd_trade_route:
        await subscribe_for_order_updates(ws, g_fcm_id, g_ib_id, g_account_id)
        await new_order(ws, g_fcm_id, g_ib_id, g_account_id, g_exchange, g_symbol, g_trade_route, g_side, quantity)

        print("#### Consume ####")
        resp = await consume(ws)
...
def place_rithmic_order(uri, system_name, user_id, password, g_exchange, g_symbol, g_side, quantity):
    ...
    return asyncio.run(place_order(uri, system_name, user_id, password, g_exchange, g_symbol, g_side, quantity))


#   ===========================================================================
if __name__ == "__main__":
    response = place_rithmic_order(
